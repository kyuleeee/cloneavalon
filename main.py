...
def main(config: EasyDict, evil_config: EasyDict):
    ...
    for i, role in enumerate(roles):
        if role in ("Assassin", "Morgana"):
            cur_player = Player(id=i + 1, role=role, role_list=roles, config=evil_config) #결국엔 evil_config
        else:
            cur_player = Player(id=i + 1, role=role, role_list=roles, config=config)
        players.append(cur_player)

    # 게임 시작
    game = Game(players, config=config, logdir=config.log_dir) #게임자체는 그냥 저렇게 하면 되는거니까! 
...
if __name__ == "__main__":
    # 직접 설정 지정
    config = EasyDict({ #착한 쪽(Arthur의 팀) 플레이어들의 설정
        "log_dir": "logs/good",
        "config_name": "ours_gpt",
        "evil_config_name": "baseline_gpt",
    }) 
    evil_config = EasyDict({  #나쁜 쪽(악당) 플레이어들의 설정
        "log_dir": "logs/evil",
